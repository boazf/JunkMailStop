...
// Constructors
    ...
    BOOL Create(UINT nBitmapID, int cx, int nGrow, COLORREF crMask)
    {
        ...
        m_hImageList = ImageList_LoadBitmap(GetApplicationHInstance(), MAKEINTRESOURCE(nBitmapID), cx, nGrow, crMask);
        return (m_hImageList != NULL) ? TRUE : FALSE;
    }
    BOOL Create(LPCTSTR lpszBitmapID, int cx, int nGrow, COLORREF crMask)
    {
        ATLASSERT(m_hImageList == NULL);
        m_hImageList = ImageList_LoadBitmap(GetApplicationHInstance(), lpszBitmapID, cx, nGrow, crMask);
        return (m_hImageList != NULL) ? TRUE : FALSE;
    }
...
// Operations
    ...
    int AddBitmap(int nNumButtons, UINT nBitmapID)
    {
        ATLASSERT(::IsWindow(m_hWnd));
        TBADDBITMAP tbab;
        tbab.hInst = _pAtlModule->GetResourceInstance();
    ...
    {
        ATLASSERT(::IsWindow(m_hWnd));
        return (int)::SendMessage(m_hWnd, TB_ADDSTRING, (WPARAM)_pAtlModule->GetResourceInstance(), (LPARAM)nStringID);
    }
...
//REVIEW - return value?
    ...
    {
        ATLASSERT(::IsWindow(m_hWnd));
        return (int)::SendMessage(m_hWnd, TB_LOADIMAGES, nBitmapID, (LPARAM)_pAtlModule->GetResourceInstance());
    }
...
// Operations
    ...
    BOOL AddTool(HWND hWnd, LPCTSTR lpszText = LPSTR_TEXTCALLBACK, LPCRECT lpRectTool = NULL, UINT nIDTool = 0)
    {
        ...
        ti.hinst = _pAtlModule->GetResourceInstance();   // needed only if lpszText is from MAKEINTRESOURCE
    ...
    void UpdateTipText(LPCTSTR lpszText, HWND hWnd, UINT nIDTool = 0)
    {
        ...
        TOOLINFO ti;
        FillInToolInfo(ti, hWnd, nIDTool);
        ti.hinst = _pAtlModule->GetResourceInstance();
        ti.lpszText = (LPTSTR)lpszText;
...
// Operations
    BOOL MakeDragList()
    {
        ...
        if(uMsg == 0)
        {
/*?*/           ::EnterCriticalSection(&_pAtlModule->m_csWindowCreate);
            if(uMsg == 0)
                uMsg = ::RegisterWindowMessage(DRAGLISTMSGSTRING);
/*?*/           ::LeaveCriticalSection(&_pAtlModule->m_csWindowCreate);
        }
        ATLASSERT(uMsg != 0);
        return bRet;
    }
